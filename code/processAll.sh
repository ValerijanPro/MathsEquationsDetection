...
do
	...
	OUT="$OUTDIR/$BNAME.lg"
	python segmenter.py -i $file -o hyp_$OUT 
	ERR=$? # test de l'erreur au cas où...
	python segmentSelect.py -o seg_$OUT  $file hyp_$OUT  
	ERR=$ERR || $?
	python symbolReco.py  -o symb_$OUT $file seg_$OUT  
	ERR=$ERR || $?
	python selectBestSeg.py -o $OUT symb_$OUT 
	ERR=$ERR || $?
